...
public class HDRParser {
	...
	public static class HDRData {
		...
		public HDRData(float[] rgb, int width, int height) {
			this.rgb = rgb;
			this.width = width;
			this.height = height;
		}
	...
	private static final Pattern widthHeightPattern = Pattern
			.compile("-Y (\\d+) \\+X (\\d+)");
	...
	public static class Header {
		...
		private Header(int valid, String programType, float gamma,
	...
	public static Header readHeader(DataInputStream in) throws IOException {
		...
		if (buf.charAt(0) == '#' && buf.charAt(1) == '?') {
			...
			if (buf == null) {
				throw new IOException(
						"Unexpected EOF reading line after magic token");
			}
		}
		...
		while (!done) {
			if (buf.equals("FORMAT=32-bit_rle_rgbe")) {
				...
				exposure = Float.parseFloat(buf.substring(exposureString
						.length()));
			} else {
	...
	public static void readPixels(DataInputStream in, float[] data,
			int numpixels) throws IOException {
		...
		while (numpixels-- > 0) {
			...
			if (numRead < 4) {
				throw new IOException("Only able to read " + numRead
						+ " out of expected " + rgbe.length + " bytes");
			}
			rgbe2float(rgb, rgbe, 0, 0);
			data[offset++] = rgb[0];
			data[offset++] = rgb[1];
			data[offset++] = rgb[2];
		}
	...
	public static void readPixelsRaw(DataInputStream in, byte[] data,
			int offset, int numpixels) throws IOException {
		int numExpected = 4 * numpixels;
		int numRead = in.read(data, offset, numExpected);
		if (numRead < numExpected)
			throw new IOException("Error reading raw pixels: got " + numRead
					+ " bytes, expected " + numExpected);
	}

	public static void readPixelsRawRLE(DataInputStream in, byte[] data,
			int offset, int scanline_width, int num_scanlines)
			throws IOException {
		byte[] rgbe = new byte[4];
		...
		while (num_scanlines > 0) {
			if (in.read(rgbe) < rgbe.length) {
				throw new IOException("Error reading bytes: expected "
						+ rgbe.length);
			}
	...
	public static void float2rgbe(byte[] rgbe, float red, float green,
			float blue) {
		float v;
	 ...
	 * ldexp(col+0.5,exp-(128+8)). However we wanted pixels in the range [0,1]
	 * to map back into the range [0,1].
	 */
	public static void rgbe2float(float[] rgb, byte[] rgbe, int rgbOffset,
			int startRGBEOffset) {
		float f;

		if (rgbe[startRGBEOffset + 3] != 0) { /* nonzero pixel */
			f = (float) ldexp(1.0, (rgbe[startRGBEOffset + 3] & 0xFF)
					- (128 + 8));
			rgb[rgbOffset + 0] = (rgbe[startRGBEOffset + 0] & 0xFF) * f;
			rgb[rgbOffset + 1] = (rgbe[startRGBEOffset + 1] & 0xFF) * f;
			rgb[rgbOffset + 2] = (rgbe[startRGBEOffset + 2] & 0xFF) * f;
		} else {
			rgb[rgbOffset + 0] = 0;
			rgb[rgbOffset + 1] = 0;
			rgb[rgbOffset + 2] = 0;
	...
	private static final double two54 = 1.80143985094819840000e+16; /*
																	 ...
																	 * 00000000
																	 */
	private static final double twom54 = 5.55111512312578270212e-17; /*
																	 * 0x3C900000
																	 * 0x00000000
																	 */
	 ...
	 * copysign(x,y) returns a value with the magnitude of x and with the sign
	 * bit of y.
	 */
	...
	public static HDRData readHDR(String file) {
		try {
			...
			in.close();
			float rgb[] = new float[header.getWidth() * header.getHeight() * 3];
			int rgbOffset = 0;
			for (int i = 0; i < data.length; i += 4) {
				rgbe2float(rgb, data, rgbOffset, i);
				rgbOffset += 3;
			}
			return new HDRData(rgb, header.getWidth(), header.getHeight());
		} catch (IOException e) {
			e.printStackTrace();
		}
		return null;
